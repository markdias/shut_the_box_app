...
final class GameStore: ObservableObject {
    ...
    func rollDice(force value: DiceRoll? = nil) {
        ...
        turnLogs.append(TurnLog(playerId: activePlayer?.id ?? UUID(), message: "Rolled \(nextRoll.values.map(String.init).joined(separator: ", "))"))
        Task { await persistSnapshot() }
    }
    ...
    private func evaluateEndOfTurn() {
        ...
        } else if GameLogic.availableCombinations(for: pendingRoll, tiles: tiles).isEmpty {
